import * as React from "react";
import { StyleSheet, View, Text } from "react-native";
import { Image } from "expo-image";
import { FontFamily, Color, Border, FontSize } from "../GlobalStyles";

const CreditOriginal1 = () => {
  return (
    <View style={styles.creditOriginal}>
      <View style={styles.eaInputBox} />
      <View
        style={[styles.creditOriginalChild, styles.creditOriginalChildLayout]}
      />
      <Text style={[styles.availableToEarn, styles.textTypo1]}>
        Available to earn interest
      </Text>
      <View style={[styles.eaInputBoxParent, styles.parentLayout]}>
        <View style={[styles.eaInputBox1, styles.inputPosition1]} />
        <Text style={[styles.supply, styles.supplyTypo]}>Supply</Text>
        <Image
          style={[styles.downloadIcon, styles.downloadIconLayout]}
          contentFit="cover"
          source={require("../assets/download1.png")}
        />
      </View>
      <View style={[styles.eaInputBoxGroup, styles.inputPosition]}>
        <View style={[styles.eaInputBox2, styles.inputPosition1]} />
        <Image
          style={[styles.downloadIcon1, styles.downloadIconLayout]}
          contentFit="cover"
          source={require("../assets/download2.png")}
        />
        <Text style={[styles.borrow, styles.homePosition]}>Borrow</Text>
      </View>
      <View
        style={[
          styles.yxldhnzg400x400RemovebgPrevParent,
          styles.yxldhnzg400x400Layout,
        ]}
      >
        <Image
          style={[
            styles.yxldhnzg400x400RemovebgPrevIcon,
            styles.yxldhnzg400x400Layout,
          ]}
          contentFit="cover"
          source={require("../assets/yxldhnzg-400x400removebgpreview-9.png")}
        />
        <Image
          style={[styles.image1Icon, styles.iconPosition]}
          contentFit="cover"
          source={require("../assets/image-1.png")}
        />
        <Image
          style={[styles.image2Icon, styles.iconPosition]}
          contentFit="cover"
          source={require("../assets/image-21.png")}
        />
      </View>
      <Text style={[styles.credit, styles.creditClr]}>Credit</Text>
      <Text style={[styles.positions, styles.creditClr]}>Positions</Text>
      <Image
        style={styles.image3Icon}
        contentFit="cover"
        source={require("../assets/image-31.png")}
      />
      <View style={[styles.homeRow, styles.homeLayout]}>
        <Image
          style={[styles.homeRowChild, styles.homeLayout]}
          contentFit="cover"
          source={require("../assets/rectangle-390.png")}
        />
        <View style={[styles.homeParent, styles.homeParentLayout]}>
          <Text style={[styles.home, styles.homePosition]}>Home</Text>
          <Image
            style={[styles.homeFillIcon, styles.fillIconLayout]}
            contentFit="cover"
            source={require("../assets/home-fill1.png")}
          />
        </View>
        <View style={[styles.darhboardParent, styles.parentPosition]}>
          <View style={[styles.darhboard, styles.fillIconLayout]}>
            <View style={[styles.darhboardChild, styles.darhboardLayout]} />
            <View style={[styles.darhboardItem, styles.darhboardPosition]} />
            <View style={[styles.darhboardInner, styles.rectangleViewLayout]} />
            <View style={[styles.rectangleView, styles.rectangleViewLayout]} />
          </View>
          <Text style={[styles.accounts, styles.accountsTypo]}>Accounts</Text>
        </View>
        <View style={[styles.exchangeParent, styles.parentPosition]}>
          <Text style={[styles.exchange, styles.accountsTypo]}>Exchange</Text>
          <Image
            style={[styles.refresh2Icon, styles.fillIconLayout]}
            contentFit="cover"
            source={require("../assets/refresh-21.png")}
          />
        </View>
        <View style={styles.groupFillParent}>
          <Image
            style={[styles.groupFillIcon, styles.fillIconLayout]}
            contentFit="cover"
            source={require("../assets/group-fill.png")}
          />
          <Text style={[styles.business, styles.supplyTypo]}>Business</Text>
        </View>
        <View style={[styles.accountParent, styles.parentPosition]}>
          <Text style={[styles.exchange, styles.accountsTypo]}>Account</Text>
          <Image
            style={[styles.userFillIcon, styles.homeParentLayout]}
            contentFit="cover"
            source={require("../assets/user-fill.png")}
          />
        </View>
      </View>
      <View style={[styles.eyeParent, styles.parentLayout]}>
        <Image
          style={styles.eyeIcon}
          contentFit="cover"
          source={require("../assets/eye.png")}
        />
        <Text style={[styles.text, styles.textTypo]}>$23,000.06</Text>
        <Image
          style={[styles.groupChild, styles.groupChildLayout]}
          contentFit="cover"
          source={require("../assets/vector-114.png")}
        />
        <Text style={[styles.creditBalance, styles.creditClr]}>
          Credit Balance
        </Text>
      </View>
      <View style={styles.listboxbgParent}>
        <View style={[styles.listboxbg, styles.listboxbgPosition]} />
        <View style={styles.placeholderText}>
          <Text style={[styles.selectAsset, styles.text1Typo]}>
            Select Asset
          </Text>
        </View>
        <Image
          style={[styles.chevronIcon, styles.groupChildLayout]}
          contentFit="cover"
          source={require("../assets/chevron1.png")}
        />
      </View>
      <View style={[styles.eaInputBoxContainer, styles.inputLayout]}>
        <View style={[styles.eaInputBox3, styles.inputLayout]} />
        <Text style={[styles.deposit, styles.supplyTypo]}>Deposit</Text>
        <Image
          style={[styles.downloadIcon2, styles.downloadIconLayout]}
          contentFit="cover"
          source={require("../assets/download2.png")}
        />
      </View>
      <View style={[styles.frameView, styles.frameViewLayout]}>
        <View style={[styles.eaInputBox4, styles.frameViewLayout]} />
        <Text style={[styles.transfer, styles.homePosition]}>Transfer</Text>
        <Image
          style={styles.twoWayArrowIcon}
          contentFit="cover"
          source={require("../assets/twoway-arrow.png")}
        />
      </View>
      <View style={styles.listboxMain}>
        <View style={[styles.listboxbg1, styles.listboxbgPosition]} />
        <Text style={[styles.text1, styles.textFlexBox]}>$ 0.00</Text>
      </View>
      <View style={[styles.eaInputBoxParent1, styles.inputParentShadowBox]}>
        <View style={[styles.eaInputBox1, styles.inputPosition1]} />
        <Text style={[styles.supply, styles.supplyTypo]}>Supply</Text>
        <Image
          style={[styles.downloadIcon, styles.downloadIconLayout]}
          contentFit="cover"
          source={require("../assets/download1.png")}
        />
      </View>
      <View style={[styles.eaInputBoxParent2, styles.inputParentShadowBox]}>
        <View style={[styles.eaInputBox2, styles.inputPosition1]} />
        <Image
          style={[styles.downloadIcon1, styles.downloadIconLayout]}
          contentFit="cover"
          source={require("../assets/download2.png")}
        />
        <Text style={[styles.borrow, styles.homePosition]}>Borrow</Text>
      </View>
      <View style={styles.listboxMain}>
        <View style={[styles.listboxbg2, styles.listboxbgPosition]} />
        <Text style={[styles.text1, styles.textFlexBox]}>$ 0.00</Text>
      </View>
      <Text style={[styles.seeTransactions, styles.creditClr]}>
        See Transactions
      </Text>
      <View style={[styles.groupParent, styles.groupPosition]}>
        <View style={[styles.listboxbgGroup, styles.selectAssetPosition]}>
          <View style={[styles.listboxbg3, styles.listboxbgShadowBox]} />
          <View style={[styles.ethParent, styles.suppliedPosition]}>
            <Text style={[styles.eth, styles.ethTypo]}>ETH</Text>
            <Text style={[styles.text3, styles.ethTypo]}>4.20%</Text>
            <Text style={[styles.text4, styles.textFlexBox]}>03/04/2024</Text>
            <Text style={[styles.text5, styles.textFlexBox]}>$ 6900.69</Text>
          </View>
        </View>
        <Text style={[styles.supplied, styles.suppliedPosition]}>Supplied</Text>
      </View>
      <View style={[styles.groupContainer, styles.groupPosition]}>
        <View style={[styles.listboxbgGroup, styles.selectAssetPosition]}>
          <View style={[styles.listboxbg4, styles.listboxbgShadowBox]} />
          <View style={[styles.ethParent, styles.suppliedPosition]}>
            <Text style={[styles.eth, styles.ethTypo]}>USDC</Text>
            <Text style={[styles.text3, styles.ethTypo]}>-6.90%</Text>
            <Text style={[styles.text4, styles.textFlexBox]}>03/04/2024</Text>
            <Text style={[styles.text5, styles.textFlexBox]}>$ 1900.42</Text>
          </View>
        </View>
        <Text style={[styles.supplied, styles.suppliedPosition]}>Borrowed</Text>
      </View>
    </View>
  );
};

const styles = StyleSheet.create({
  creditOriginalChildLayout: {
    width: 390,
    left: 0,
  },
  textTypo1: {
    fontFamily: FontFamily.interLight,
    fontWeight: "300",
  },
  parentLayout: {
    height: 58,
    position: "absolute",
  },
  inputPosition1: {
    backgroundColor: Color.colorGhostwhite,
    left: 0,
    top: 0,
    borderRadius: Border.br_3xs,
  },
  supplyTypo: {
    textAlign: "center",
    top: 34,
    fontSize: FontSize.size_xs,
    position: "absolute",
  },
  downloadIconLayout: {
    height: 22,
    width: 25,
    top: 7,
    position: "absolute",
  },
  inputPosition: {
    left: 230,
    width: 136,
  },
  homePosition: {
    top: 35,
    textAlign: "center",
    fontSize: FontSize.size_xs,
    position: "absolute",
  },
  yxldhnzg400x400Layout: {
    height: 62,
    position: "absolute",
  },
  iconPosition: {
    height: 44,
    top: 9,
    position: "absolute",
  },
  creditClr: {
    color: Color.colorDimgray_300,
    textAlign: "left",
  },
  homeLayout: {
    height: 87,
    width: 390,
    left: 0,
    position: "absolute",
  },
  homeParentLayout: {
    width: 35,
    position: "absolute",
  },
  fillIconLayout: {
    height: 35,
    width: 35,
    top: 0,
    position: "absolute",
  },
  parentPosition: {
    height: 48,
    top: 21,
    position: "absolute",
  },
  darhboardLayout: {
    borderWidth: 2,
    borderColor: Color.colorDarkslateblue_100,
    borderRadius: Border.br_12xs,
    left: "13.71%",
    right: "55.43%",
    width: "30.86%",
    height: "30.86%",
    borderStyle: "solid",
    position: "absolute",
  },
  darhboardPosition: {
    bottom: "13.71%",
    top: "55.43%",
  },
  rectangleViewLayout: {
    left: "55.43%",
    right: "13.71%",
    borderWidth: 2,
    borderColor: Color.colorDarkslateblue_100,
    borderRadius: Border.br_12xs,
    width: "30.86%",
    height: "30.86%",
    borderStyle: "solid",
    position: "absolute",
  },
  accountsTypo: {
    top: 33,
    textAlign: "center",
    fontFamily: FontFamily.interLight,
    fontWeight: "300",
    fontSize: FontSize.size_xs,
    left: 0,
    position: "absolute",
  },
  textTypo: {
    fontFamily: FontFamily.interMedium,
    fontWeight: "500",
    position: "absolute",
  },
  groupChildLayout: {
    maxHeight: "100%",
    overflow: "hidden",
    maxWidth: "100%",
    position: "absolute",
  },
  listboxbgPosition: {
    borderRadius: Border.br_5xs,
    shadowOpacity: 1,
    shadowOffset: {
      width: 0,
      height: 2,
    },
    left: "0%",
    bottom: "0%",
    top: "0%",
    height: "100%",
    right: "0%",
    backgroundColor: Color.colorGhostwhite,
    position: "absolute",
    width: "100%",
  },
  text1Typo: {
    color: Color.colorDimgray_200,
    fontFamily: FontFamily.interRegular,
    fontSize: FontSize.size_base,
  },
  inputLayout: {
    width: 164,
    height: 58,
    position: "absolute",
  },
  frameViewLayout: {
    width: 167,
    height: 58,
    position: "absolute",
  },
  textFlexBox: {
    textAlign: "right",
    position: "absolute",
  },
  inputParentShadowBox: {
    shadowOpacity: 1,
    shadowOffset: {
      width: 0,
      height: 2,
    },
    elevation: 4,
    shadowRadius: 4,
    shadowColor: "rgba(0, 0, 0, 0.25)",
    height: 58,
    top: 406,
    position: "absolute",
  },
  groupPosition: {
    left: "7.18%",
    right: "7.18%",
    width: "85.64%",
    height: "8.41%",
    position: "absolute",
  },
  selectAssetPosition: {
    left: "0%",
    top: "0%",
    position: "absolute",
  },
  listboxbgShadowBox: {
    elevation: 15,
    shadowRadius: 15,
    borderRadius: Border.br_5xs,
    shadowOpacity: 1,
    shadowOffset: {
      width: 0,
      height: 2,
    },
    shadowColor: "rgba(0, 0, 0, 0.25)",
    left: "0%",
    bottom: "0%",
    top: "0%",
    height: "100%",
    right: "0%",
    position: "absolute",
    width: "100%",
  },
  suppliedPosition: {
    left: 13,
    position: "absolute",
  },
  ethTypo: {
    width: 61,
    color: Color.colorDimgray_600,
    textAlign: "left",
    fontFamily: FontFamily.interLight,
    fontWeight: "300",
    position: "absolute",
  },
  eaInputBox: {
    top: 431,
    left: 63,
    borderColor: Color.colorDarkgray_600,
    borderWidth: 1,
    width: 161,
    height: 93,
    borderStyle: "solid",
    borderRadius: Border.br_3xs,
    backgroundColor: Color.colorWhite,
    position: "absolute",
  },
  creditOriginalChild: {
    top: 0,
    backgroundColor: Color.colorWhite,
    width: 390,
    position: "absolute",
    height: 844,
  },
  availableToEarn: {
    top: 167,
    color: Color.colorDimgray_400,
    width: 201,
    textAlign: "left",
    fontSize: FontSize.size_xs,
    left: 25,
    position: "absolute",
  },
  eaInputBox1: {
    height: 58,
    position: "absolute",
    width: 201,
  },
  supply: {
    left: 72,
    height: 17,
    width: 53,
    color: Color.colorRoyalblue_100,
    fontFamily: FontFamily.interMedium,
    fontWeight: "500",
  },
  downloadIcon: {
    left: 86,
  },
  eaInputBoxParent: {
    left: 22,
    top: 406,
    height: 58,
    width: 201,
  },
  eaInputBox2: {
    width: 136,
    height: 58,
    position: "absolute",
  },
  downloadIcon1: {
    left: 55,
  },
  borrow: {
    left: 42,
    height: 17,
    width: 53,
    color: Color.colorRoyalblue_100,
    fontFamily: FontFamily.interMedium,
    fontWeight: "500",
  },
  eaInputBoxGroup: {
    width: 136,
    height: 58,
    position: "absolute",
    top: 406,
  },
  yxldhnzg400x400RemovebgPrevIcon: {
    left: 165,
    width: 58,
    top: 0,
  },
  image1Icon: {
    width: 48,
    left: 0,
  },
  image2Icon: {
    left: 291,
    width: 99,
  },
  yxldhnzg400x400RemovebgPrevParent: {
    top: 31,
    width: 390,
    left: 0,
  },
  credit: {
    top: 291,
    fontSize: FontSize.size_xl,
    color: Color.colorDimgray_300,
    fontFamily: FontFamily.interMedium,
    fontWeight: "500",
    position: "absolute",
    left: 24,
  },
  positions: {
    top: 490,
    fontSize: FontSize.size_xl,
    color: Color.colorDimgray_300,
    fontFamily: FontFamily.interMedium,
    fontWeight: "500",
    position: "absolute",
    left: 25,
  },
  image3Icon: {
    top: 130,
    width: 32,
    height: 33,
    left: 22,
    position: "absolute",
  },
  homeRowChild: {
    top: 0,
  },
  home: {
    color: Color.colorDimgray_600,
    fontFamily: FontFamily.interLight,
    fontWeight: "300",
    left: 0,
  },
  homeFillIcon: {
    left: 0,
  },
  homeParent: {
    top: 19,
    height: 50,
    left: 24,
  },
  darhboardChild: {
    bottom: "55.43%",
    top: "13.71%",
  },
  darhboardItem: {
    borderWidth: 2,
    borderColor: Color.colorDarkslateblue_100,
    borderRadius: Border.br_12xs,
    left: "13.71%",
    right: "55.43%",
    width: "30.86%",
    height: "30.86%",
    borderStyle: "solid",
    position: "absolute",
  },
  darhboardInner: {
    bottom: "13.71%",
    top: "55.43%",
  },
  rectangleView: {
    bottom: "55.43%",
    top: "13.71%",
  },
  darhboard: {
    left: 9,
  },
  accounts: {
    color: Color.colorDarkslateblue_200,
  },
  darhboardParent: {
    left: 87,
    width: 53,
  },
  exchange: {
    color: Color.colorDimgray_600,
  },
  refresh2Icon: {
    left: 10,
  },
  exchangeParent: {
    left: 168,
    width: 55,
  },
  groupFillIcon: {
    left: 8,
  },
  business: {
    color: Color.colorDimgray_600,
    fontFamily: FontFamily.interLight,
    fontWeight: "300",
    left: 0,
  },
  groupFillParent: {
    top: 20,
    left: 246,
    width: 50,
    height: 49,
    position: "absolute",
  },
  userFillIcon: {
    left: 6,
    height: 33,
    top: 0,
  },
  accountParent: {
    left: 319,
    width: 47,
  },
  homeRow: {
    top: 757,
  },
  eyeIcon: {
    top: 1,
    left: 100,
    width: 13,
    height: 13,
    position: "absolute",
  },
  text: {
    top: 24,
    left: 29,
    fontSize: FontSize.size_9xl,
    color: Color.colorDarkslateblue_100,
    textAlign: "left",
  },
  groupChild: {
    height: "27.59%",
    width: "2.99%",
    top: "56.9%",
    bottom: "15.52%",
    left: "97.01%",
    right: "0%",
    maxHeight: "100%",
    overflow: "hidden",
    maxWidth: "100%",
  },
  creditBalance: {
    fontFamily: FontFamily.interMedium,
    fontWeight: "500",
    position: "absolute",
    fontSize: FontSize.size_xs,
    left: 0,
    top: 0,
  },
  eyeParent: {
    top: 106,
    width: 201,
    left: 25,
  },
  listboxbg: {
    elevation: 4,
    shadowRadius: 4,
    shadowColor: "rgba(0, 0, 0, 0.25)",
    borderRadius: Border.br_5xs,
  },
  selectAsset: {
    left: "0%",
    top: "0%",
    position: "absolute",
    fontFamily: FontFamily.interRegular,
    fontSize: FontSize.size_base,
    textAlign: "left",
  },
  placeholderText: {
    height: "33.33%",
    width: "65.72%",
    right: "28.56%",
    bottom: "33.33%",
    left: "5.72%",
    top: "33.33%",
    position: "absolute",
  },
  chevronIcon: {
    height: "8.33%",
    width: "6.92%",
    top: "46.67%",
    right: "10.9%",
    bottom: "45%",
    left: "82.19%",
    opacity: 0.8,
  },
  listboxbgParent: {
    top: 330,
    height: 60,
    left: 22,
    width: 201,
    position: "absolute",
  },
  eaInputBox3: {
    backgroundColor: Color.colorGhostwhite,
    left: 0,
    top: 0,
    borderRadius: Border.br_3xs,
  },
  deposit: {
    left: 56,
    height: 17,
    width: 53,
    color: Color.colorRoyalblue_100,
    fontFamily: FontFamily.interMedium,
    fontWeight: "500",
  },
  downloadIcon2: {
    left: 70,
  },
  eaInputBoxContainer: {
    top: 208,
    shadowOpacity: 1,
    elevation: 4,
    shadowRadius: 4,
    shadowOffset: {
      width: 0,
      height: 2,
    },
    shadowColor: "rgba(0, 0, 0, 0.25)",
    left: 25,
  },
  eaInputBox4: {
    backgroundColor: Color.colorGhostwhite,
    left: 0,
    top: 0,
    borderRadius: Border.br_3xs,
  },
  transfer: {
    left: 57,
    height: 17,
    width: 53,
    color: Color.colorRoyalblue_100,
    fontFamily: FontFamily.interMedium,
    fontWeight: "500",
  },
  twoWayArrowIcon: {
    top: 11,
    left: 73,
    height: 18,
    width: 25,
    position: "absolute",
  },
  frameView: {
    left: 199,
    top: 208,
    shadowOpacity: 1,
    elevation: 4,
    shadowRadius: 4,
    shadowOffset: {
      width: 0,
      height: 2,
    },
    shadowColor: "rgba(0, 0, 0, 0.25)",
  },
  listboxbg1: {
    shadowColor: "rgba(0, 0, 0, 0.1)",
    shadowRadius: 14,
    elevation: 14,
  },
  text1: {
    width: "83.82%",
    left: "11.03%",
    color: Color.colorDimgray_200,
    fontFamily: FontFamily.interRegular,
    fontSize: FontSize.size_base,
    top: "33.33%",
  },
  listboxMain: {
    height: "7.11%",
    width: "34.87%",
    top: "39.1%",
    right: "6.15%",
    bottom: "53.79%",
    left: "58.97%",
    position: "absolute",
  },
  eaInputBoxParent1: {
    left: 22,
    width: 201,
  },
  eaInputBoxParent2: {
    width: 136,
    left: 230,
  },
  listboxbg2: {
    elevation: 4,
    shadowRadius: 4,
    shadowColor: "rgba(0, 0, 0, 0.25)",
    borderRadius: Border.br_5xs,
  },
  seeTransactions: {
    top: 301,
    left: 238,
    fontWeight: "600",
    fontFamily: FontFamily.interSemiBold,
    fontSize: FontSize.size_sm,
    position: "absolute",
  },
  listboxbg3: {
    backgroundColor: "#c5ffe9",
  },
  eth: {
    fontSize: FontSize.size_sm,
    left: 0,
    top: 0,
  },
  text3: {
    top: 4,
    left: 51,
    fontSize: FontSize.size_3xs,
  },
  text4: {
    left: 136,
    width: 83,
    fontSize: FontSize.size_sm,
    color: Color.colorDimgray_600,
    fontFamily: FontFamily.interLight,
    fontWeight: "300",
    top: 0,
  },
  text5: {
    left: 219,
    width: 81,
    fontSize: FontSize.size_sm,
    color: Color.colorDimgray_600,
    fontFamily: FontFamily.interLight,
    fontWeight: "300",
    top: 0,
  },
  ethParent: {
    top: 36,
    width: 300,
    height: 17,
  },
  listboxbgGroup: {
    bottom: "0%",
    height: "100%",
    left: "0%",
    top: "0%",
    right: "0%",
    width: "100%",
  },
  supplied: {
    fontSize: FontSize.size_sm,
    color: Color.colorDimgray_300,
    textAlign: "left",
    top: 7,
    left: 13,
    fontFamily: FontFamily.interMedium,
    fontWeight: "500",
  },
  groupParent: {
    top: "61.97%",
    bottom: "29.62%",
  },
  listboxbg4: {
    backgroundColor: "#ffc6c6",
  },
  groupContainer: {
    top: "73.93%",
    bottom: "17.65%",
  },
  creditOriginal: {
    flex: 1,
    height: 844,
    width: "100%",
  },
});

export default CreditOriginal1;
